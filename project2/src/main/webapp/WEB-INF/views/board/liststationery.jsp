	...
	<div class="container">
		...
		<section class="content">
			...
			<table width="800px"
				...
				<th colspan="4"
					style="height: 40px; vertical-align: top; text-align: left;">Stationery</th>
			...
			<nav>
				...
				<ul>
					...
					<li><a href="/Stationery"><b>Stationery</b></a></li>
					<li><a href="/Etc">etc.</a></li>
				</ul>
				<div style="position: relative; bottom: 0px; text-align: center;">
					<c:choose>
						<c:when test="${CurNum eq 1}">
							<span><a href="/Stationery?num=${CurNum}">&lt;&lt;</a></span>
						</c:when>
						<c:otherwise>
							<span><a href="/Stationery?num=${CurNum-1}">&lt;&lt;</a></span>
						</c:otherwise>
					</c:choose>
					<c:forEach begin="${StartNum}" end="${EndNum}" var="num">
						<c:choose>
							<c:when test="${CurNum eq num}">
								<span> <a href="/Stationery?num=${num}"><b>${num}</b></a>
								</span>
							</c:when>
							<c:when test="${num le pageNum}">
								<span> <a href="/Stationery?num=${num}">${num}</a>
								</span>
							</c:when>
						</c:choose>
					</c:forEach>
