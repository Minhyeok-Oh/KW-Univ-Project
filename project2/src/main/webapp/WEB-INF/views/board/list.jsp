	...
	<div class="container">
		...
		<section class="content">
			...
			<table width="800px"
				...
				<th colspan="4"
					style="height: 40px; vertical-align: top; text-align: left;">ALL</th>
				...
				<c:forEach items="${list}" var="list">
					...
					<tr>
						<th rowspan="3">
							...
							<img src="/resources/img/${fileList.stored_file_name}" style="max-width: 100%"; height:auto; alt="오류">
							</c:if>
							</c:forEach>
						</th>
						<td style="height: 20px;">상품설명</td>
						<td>가격</td>
						<td rowspan="2">PostNumber: ${list.bno}<br>이름:${list.writer}<br>조회수:${list.viewCnt}<br>작성일:<fmt:formatDate value="${list.regDate}" pattern ="yyyy.MM.dd"/></div>
						</td>
					</tr>
					<tr>
						<td rowspan="2" style="height: 100px;">${list.description}</td>
						<td rowspan="2"><fmt:formatNumber type="number" maxFractionDigits="3" value="${list.price}" />￦</td>
					</tr>
					<tr>
			...
			<nav>
				...
				<div style="text-align:center;">
					...
					<span><a href="/?num=${CurNum}">&lt;&lt;</a></span>
					</c:when>
					<c:otherwise>
					<span><a href="/?num=${CurNum-1}">&lt;&lt;</a></span>
					</c:otherwise>				
					</c:choose>
					<c:forEach begin="${StartNum}" end="${EndNum}" var="num">
						<c:choose>
						<c:when test="${CurNum eq num}">
						<span> <a href="/?num=${num}"><b>${num}</b></a>
						</span>
						</c:when>
						<c:when test="${num le pageNum}">
						<span> <a href="/?num=${num}">${num}</a>
						</span>
						</c:when>
						</c:choose>
					</c:forEach>
