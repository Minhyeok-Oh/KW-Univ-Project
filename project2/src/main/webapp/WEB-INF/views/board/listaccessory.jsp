	...
	<div class="container">
		...
		<section class="content">
			...
			<table width="800px"
				...
				<th colspan="4"
					style="height: 40px; vertical-align: top; text-align: left;">Accessory</th>
			...
			<nav>
				...
				<ul>
					...
					<li><a href="/Accessory"><strong>Accessory</strong></a></li>
				...
				<div style="position: relative; bottom: 0px; text-align: center;">
					<c:choose>
						<c:when test="${CurNum eq 1}">
							<span><a href="/Accessory?num=${CurNum}">&lt;&lt;</a></span>
						</c:when>
						<c:otherwise>
							<span><a href="/Accessory?num=${CurNum-1}">&lt;&lt;</a></span>
						</c:otherwise>
					</c:choose>
					<c:forEach begin="${StartNum}" end="${EndNum}" var="num">
						<c:choose>
							<c:when test="${CurNum eq num}">
								<span> <a href="/Accessory?num=${num}"><b>${num}</b></a>
								</span>
							</c:when>
							<c:when test="${num le pageNum}">
								<span> <a href="/Accessory?num=${num}">${num}</a>
								</span>
							</c:when>
						</c:choose>
					</c:forEach>
